      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4 max-w-2xl mx-auto">
        {LINKS.map((link) => (
          <a
            ...
            className="flex-1 border border-white/10 rounded-xl p-4 bg-white/5 backdrop-blur-xl hover:border-blue-500/50 transition-all"
          >
            <span className="block text-sm text-blue-300 mb-1">{link.label}</span>
            <span className="block text-white/70 text-sm break-words">{link.value}</span>
          </a>
        ))}
      </div>
    </section>
